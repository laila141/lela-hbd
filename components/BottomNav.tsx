...
const BottomNav: React.FC<BottomNavProps> = ({ currentView, setView }) => {
  const navItems = [
    { view: View.GALLERY, icon: 'gallery_thumbnail', label: 'Gallery' },
    { view: View.COUPONS, icon: 'confirmation_number', label: 'Coupons' },
    { view: View.LETTER, icon: 'drafts', label: 'Letter' },
  ];
        ...
        {navItems.map((item) => (
          ...
          >
            <span 
              ...
              style={{ fontVariationSettings: currentView === item.view ? "'FILL' 1" : "'FILL' 0" }}
            >
              {item.icon}
            </span>
        ...
        {navItems.map((item) => (
          <button
            ...
            className={`group flex flex-col items-center gap-2 p-3 rounded-xl transition-all hover:bg-primary/5 ${
              currentView === item.view ? 'text-primary bg-primary/10' : 'text-[#8a6072]'
            }`}
            title={item.label}
          >
            <span 
