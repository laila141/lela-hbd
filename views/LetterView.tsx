...
const LetterView: React.FC = () => {
  ...
  return (
    <div className="relative flex h-auto min-h-screen w-full flex-col overflow-x-hidden overflow-y-auto pb-24"
         style={{ background: 'radial-gradient(circle at top right, rgba(244, 37, 123, 0.1) 0%, transparent 40%), radial-gradient(circle at bottom left, rgba(244, 37, 123, 0.1) 0%, transparent 40%)' }}>
      ...
      <div className="flex items-center bg-transparent p-4 pb-2 justify-between z-10">
        ...
        <h2 className="text-[#181114] dark:text-white text-lg font-bold leading-tight flex-1 text-center">For My Special One</h2>
      ...
      <main className="flex-1 flex flex-col items-center justify-start pt-10 px-6">
        ...
        <div className={`relative w-full max-w-sm aspect-[4/3] transition-all duration-700 perspective-1000 ${isOpen ? 'mt-32 mb-10' : 'mt-10 mb-20'}`}>
          <div className="relative w-full h-full preserve-3d">
              ...
              <div 
                ...
                ${isOpen ? 'bottom-[30%] h-[120%] -rotate-1' : 'bottom-[10%] h-[80%] translate-y-0 rotate-0'}`}
                style={{ backgroundImage: 'url(https://lh3.googleusercontent.com/aida-public/AB6AXuBU873eYPqL_B5gUTmNcm829EJQIxRbI_82U1w9f1WudTQKjYmoE6vmHbvNxv64aTqFWiq9Unb6IUzfbn0Nuu4cQdmfrtwIetAT0DuTqRMPwSh1jsMLh-isOH76-Cj1pzNTTG_1SV6VbAUaLHkH-g9uzAey34OrWYvFRYPYYYL9Avn8JaDqHuf5qSfUDp9qWkSLXqc65kavABThOvlyRqhTNtsWucBaW64HUsjVvSdfaC1mC8TPO6l0lsx9Nm84WiJfLwcOpMawZAxw)' }}
              >
                  {!isOpen && <span className="material-symbols-outlined text-6xl text-primary/20 mt-10">favorite</span>}
                  
                  {isOpen && (
                      <div className="w-full h-full overflow-hidden text-center opacity-0 animate-[fadeIn_1s_ease-out_0.5s_forwards]">
                          <h3 className="font-handwritten text-xl text-primary mb-2">My Dearest,</h3>
                          <div className="h-1 w-full bg-primary/10 mb-2"></div>
                          <div className="h-1 w-3/4 bg-primary/10 mx-auto mb-2"></div>
                          <div className="h-1 w-full bg-primary/10 mb-2"></div>
                          <p className="text-xs text-[#8a6072] mt-4 font-serif italic">Tap "Read Full Letter" below</p>
                      </div>
                  )}
              </div>

              {/* Envelope Front (Bottom Pocket) */}
              <div className="absolute bottom-0 left-0 w-full h-1/2 bg-white dark:bg-[#2d1a22] z-20 rounded-b-xl border-t border-primary/5 shadow-inner" style={{ clipPath: 'polygon(0 0, 50% 40%, 100% 0, 100% 100%, 0 100%)' }}></div>
              ...
              {/* Wax Seal Button */}
              <div className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-40 transition-opacity duration-300 ${isOpen ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}>
                <button 
                ...
                >
                  <span className="material-symbols-outlined text-3xl" style={{ fontVariationSettings: "'FILL' 1" }}>heart_check</span>
                </button>
              </div>
          </div>
        </div>
        ...
        {!isOpen && (
            <div className="text-center animate-fade-in mt-4">
                <h4 className="text-primary text-sm font-bold uppercase tracking-widest px-4 py-2">Tap the heart to open</h4>
                <p className="text-[#8a6072] dark:text-[#c4a1b0] text-sm mt-1">A birthday surprise is waiting inside...</p>
            </div>
        )}

        {/* Read Full Letter Button */}
        <div className={`transition-all duration-700 delay-500 ${isOpen ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10 pointer-events-none'} w-full`}>
             <div className="parchment-texture w-full max-w-2xl mx-auto rounded-xl shadow-2xl overflow-hidden border border-[#e6dbdf] relative bg-parchment">
                <div className="px-8 py-10 text-[#2d1a22]">
                    <h1 className="font-handwritten text-4xl text-primary text-center mb-8">Happy Birthday, My Love!</h1>
                    <div className="space-y-6 font-handwritten text-2xl leading-relaxed">
                        <p>Dearest,</p>
                        <p>Today is a celebration of the most wonderful person I know. Every day with you feels like a beautiful gift, but today is extra special because it's the day the world got a little brighter because of you.</p>
                        <p>I hope your day is filled with as much joy, laughter, and love as you bring into my life every single day.</p>
                        <div className="w-full flex justify-center py-4">
                            <span className="material-symbols-outlined text-primary text-4xl" style={{ fontVariationSettings: "'FILL' 1" }}>favorite</span>
                        </div>
                        <p>Thank you for being your incredible self.</p>
                        <p className="pt-4">With all my love,</p>
                        <p className="text-3xl font-bold">Always Yours ❤️</p>
                    </div>
                ...
                {/* Decorative corner */}
                <img alt="decoration" className="absolute bottom-0 right-0 w-32 h-32 opacity-10 pointer-events-none" src="https://lh3.googleusercontent.com/aida-public/AB6AXuBtHl6Rjqn-_SHmwIqnfr1HrY9mX1JVeoGMPjlEKapcZ1rAP7USsWbzXY3UmcZky9f745TsfRrfOCMin8XY7nMt0kEHQ9ZHAA6Gh2dJGkaUfOFJBE4gH1EB4m4gO5tqybkKp2mBn8HOglTRjw18p0G0ls8-joORpS2oB6dyq2G_HU2PViwvuGj9B4s9yfCq63-jRaHetwm5F0EkCXOLd2ZzaZ91FNInVmfk4xqPq180Rcyo2-fhlsFhxt0JLRIig2HVkcYYqu_1_ALL" />
             </div>
        </div>
      </main>
    </div>
